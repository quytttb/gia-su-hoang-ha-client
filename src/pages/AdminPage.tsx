...
const AdminPage = () => {
     ...
     const handleDeleteCourse = (courseId: string) => {
          ...
          if (hasRegistrations) {
               alert('Không thể xóa khóa học này vì đã có học viên đăng ký. Vui lòng hủy các đăng ký trước.');
               return;
          }
               ...
               setCourses(prevCourses =>
                    prevCourses.map(course =>
                         course.id === editingCourse.id
                              ? { ...course, ...courseFormData } as Course
                              : course
                    )
               );
          } else {
     ...
     const handleSubmitSchedule = (e: FormEvent) => {
          ...
          // Basic validation
          if (!scheduleFormData.courseId || !scheduleFormData.date || !scheduleFormData.startTime || !scheduleFormData.endTime) {
               alert('Vui lòng điền đầy đủ thông tin bắt buộc.');
               return;
          }
          ...
          <Layout>
               ...
               <section className="bg-gray-100 py-8">
                    <div className="container-custom">
                         ...
                         <p className="text-gray-600">Quản lý khóa học, lịch học, đăng ký và phản hồi của học viên</p>
                    </div>
               </section>
                                                  ...
                                                  <div className="overflow-x-auto">
                                                       <table className="w-full">
                                                            <thead className="bg-gray-50">
                                                                 <tr>
                                                                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Tên khóa học</th>
                                                                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Danh mục</th>
                                                                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Đối tượng</th>
                                                                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Học phí</th>
                                                            ...
                                                            <tbody className="divide-y divide-gray-200">
                                                                 {courses.map(course => (
                                                                      <tr key={course.id} className="hover:bg-gray-50">
                                                                           ...
                                                                           <td className="px-4 py-3 text-sm text-gray-600">{course.targetAudience}</td>
                                                                           <td className="px-4 py-3 text-sm text-gray-600">{formatCurrency(course.price)}</td>
                                                                           <td className="px-4 py-3 text-sm">
                                                                                <span className={`px-2 py-1 rounded-full text-xs ${course.featured ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                                       ...
                                                       <table className="w-full">
                                                            <thead className="bg-gray-50">
                                                                 <tr>
                                                                      ...
                                                                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Ngày</th>
                                                                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Thời gian</th>
                                                                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Giáo viên</th>
                                                                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Phòng</th>
                                                                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Số học viên</th>
                                                                      <th className="px-4 py-3 text-right text-sm font-medium text-gray-500">Thao tác</th>
                                                                 </tr>
                                                            </thead>
                                                            <tbody className="divide-y divide-gray-200">
                                                                 {schedules.map(schedule => (
                                                                      <tr key={schedule.id} className="hover:bg-gray-50">
                                                                           <td className="px-4 py-3 text-sm font-medium text-gray-800">{schedule.courseName}</td>
                                                                           <td className="px-4 py-3 text-sm text-gray-600">{formatDate(schedule.date)}</td>
                                                                           <td className="px-4 py-3 text-sm text-gray-600">{schedule.startTime} - {schedule.endTime}</td>
                                                                           ...
                                                                           <td className="px-4 py-3 text-sm text-gray-600">{schedule.studentIds.length}</td>
                                                                           <td className="px-4 py-3 text-sm text-right">
                                                                                <button
                                                                                     className="text-blue-600 hover:text-blue-800 mr-3"
                                        ...
                                        <div className="bg-white rounded-lg shadow-md p-6">
                                             ...
                                             <div className="overflow-x-auto">
                                                  <table className="w-full">
                                                       <thead className="bg-gray-50">
                                                            <tr>
                                                                 <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Mã đăng ký</th>
                                                                 <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Học viên</th>
                                                                 <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Khóa học</th>
                                                                 <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Ngày đăng ký</th>
                                                                 <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Trạng thái</th>
                                                                 <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Thanh toán</th>
                                                                 <th className="px-4 py-3 text-right text-sm font-medium text-gray-500">Thao tác</th>
                                                            </tr>
                                                       </thead>
                                                       <tbody className="divide-y divide-gray-200">
                                                            {registrations.map(registration => {
                                                                 ...
                                                                 return (
                                                                      <tr key={registration.id} className="hover:bg-gray-50">
                                                                           <td className="px-4 py-3 text-sm font-medium text-gray-800">{registration.id}</td>
                                                                           ...
                                                                           <td className="px-4 py-3 text-sm text-gray-600">{formatDate(registration.registrationDate)}</td>
                                                                           <td className="px-4 py-3 text-sm">
                                                                                <span className={`px-2 py-1 rounded-full text-xs ${getStatusColor(registration.status)}`}>
                                                                                     {registration.status === 'approved' ? 'Đã xác nhận' :
                                                                                          registration.status === 'pending' ? 'Đang chờ' : 'Đã hủy'}
                                                                                </span>
                                                                           </td>
                                                                           <td className="px-4 py-3 text-sm">
                                                                                <span className={`px-2 py-1 rounded-full text-xs ${getStatusColor(registration.paymentStatus)}`}>
                                                                                     {registration.paymentStatus === 'completed' ? 'Đã thanh toán' : 'Chưa thanh toán'}
                                                                                </span>
                                                                           </td>
                                                                           <td className="px-4 py-3 text-sm text-right">
                                                            ...
                                                            <div key={inquiry.id} className="border rounded-lg p-4">
                                                                 <div className="flex justify-between items-start mb-3">
                                                                      ...
                                                                      <span className={`px-2 py-1 rounded-full text-xs ${getStatusColor(inquiry.status)}`}>
                                                                           {inquiry.status === 'new' ? 'Mới' :
                                                                                inquiry.status === 'in-progress' ? 'Đang xử lý' : 'Đã xử lý'}
                                                                      </span>
                                                                 </div>

                                                                 <div className="bg-gray-50 p-3 rounded mb-3">
                                                                      <p className="text-gray-700 whitespace-pre-line">{inquiry.message}</p>
                                                                      <p className="text-xs text-gray-500 mt-2">Gửi lúc: {formatDate(inquiry.date)}</p>
                                                                 </div>
