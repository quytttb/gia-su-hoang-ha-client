...
import { TutorTable, TutorForm } from '../../components/admin/tutors';
     ...
     const handleSaveTutor = async (tutorData: Omit<Tutor, 'id'>) => {
          try {
               ...
               if (editingTutor) {
                    ...
                    setFilteredTutors(prev => prev.map(t => t.id === editingTutor.id ? { ...t, ...tutorData } : t));
               } else {
                    // Create
                    const newId = await tutorsService.addTutor({ ...tutorData, isActive: true, experience: '', education: '', subjects: [], availability: [] });
                    if (newId) {
               ...
               <div className="space-y-6">
                    ...
                    <div className="bg-card rounded-lg p-6 border flex items-center justify-between">
                         <div>
                              ...
                              <p className="text-muted-foreground">Thêm, chỉnh sửa và quản lý đội ngũ giáo viên của trung tâm.</p>
                         </div>
                         <Button onClick={() => { setEditingTutor(undefined); setIsFormOpen(true); }}>
                    ...
                    <TutorForm
                         tutor={editingTutor}
                         isOpen={isFormOpen}
                         onClose={() => { setIsFormOpen(false); setEditingTutor(undefined); }}
                    ...
                    <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
                         <DialogContent>
                              <DialogHeader>
                                   ...
                                   <DialogDescription>
                                        Bạn có chắc chắn muốn xóa giáo viên "{tutorToDelete?.name}"? Hành động này không thể hoàn tác.
                                   </DialogDescription>
                              </DialogHeader>
                              <DialogFooter>
                                   <Button
